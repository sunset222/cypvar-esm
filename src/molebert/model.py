...
class FT_Projector(torch.nn.Module):
    def __init__(self, emb_dim):
        super(FT_Projector, self).__init__()
        self.proj1 = nn.Linear(emb_dim, 128)

    def forward(self, x_ft):
        return self.proj1(x_ft)
...
class GNN_graphpred(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, num_tasks, JK = "last", drop_ratio = 0, graph_pooling = "mean", gnn_type = "gin"):
        ...
        if self.num_layer < 2:
            raise ValueError("Number of GNN layers must be greater than 1.")
        #self.gnn_nf = GNN(num_layer, emb_dim, JK, drop_ratio, gnn_type = gnn_type)
        self.gnn_ft = GNN(num_layer, emb_dim, JK, drop_ratio, gnn_type = gnn_type)
        #self.proj = FT_Projector(emb_dim)
        ...
        # 각 태스크에 대해 별도의 예측 레이어 생성
        if self.JK == "concat":
            input_dim = self.mult * (self.num_layer + 1) * self.emb_dim
        else:
            input_dim = self.mult * self.emb_dim
        ...
        for _ in range(self.num_tasks):
            task_predictor = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(self.emb_dim, 1)  # 각 태스크는 하나의 출력을 가짐
            )
            self.task_predictors.append(task_predictor)

    def from_pretrained(self, model_file):
        #self.gnn = GNN(self.num_layer, self.emb_dim, JK = self.JK, drop_ratio = self.drop_ratio)
        #self.gnn_nf.load_state_dict(torch.load(model_file))
        self.gnn_ft.load_state_dict(torch.load(model_file))
    ...
    def forward(self, *argv):
        ...
        #node_nf = self.gnn_nf(x, edge_index, edge_attr)
        node_ft = self.gnn_ft(x, edge_index, edge_attr)
        #node_projected = self.proj(node_ft)
        
        pooled = self.pool(node_ft, batch)
        
        # 각 태스크에 대한 예측 수행
        predictions = []
        for task_predictor in self.task_predictors:
            task_pred = task_predictor(pooled)
            predictions.append(task_pred)
            
        # 모든 태스크의 예측을 결합 (shape: [batch_size, num_tasks])
        predictions = torch.cat(predictions, dim=1)
        
        return predictions, node_ft
